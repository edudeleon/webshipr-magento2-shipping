...
class Close extends \Magento\Framework\App\Action\Action
{
    ...
    {   
        ...
        if (!$order->canShip()) {
            $this->_return_http_response(
                __("Order can't be shipped automatically. Please close/complete the order from Magento admin panel."),
                false
            );
        }

        if ($this->_hasItemsToInvoice($order) && !$order->canInvoice()) {
            $this->_return_http_response(
                __("Order can't be invoiced automatically. Please close/complete the order from Magento admin panel."),
                false
            );
        }
        ...
        try {
            ...
            if(in_array('notify_shipment', $this->_webshiprHelper->getEnabledNotifications())){
            ...
            $order->addStatusHistoryComment(
                __('Shipment order automatically generated by Webshipr. Shipment #%1', $shipment->getIncrementId()))
            ->save();
        ...
        if ($this->_hasItemsToInvoice($order)) {
    ...
    protected function _invoiceOrder(Order $order)
    {
        ...
        if (in_array('notify_invoice', $this->_webshiprHelper->getEnabledNotifications())) {
            $this->_invoiceSender->send($invoice);
        }

        // Include Invoice comment
        $order->addStatusHistoryComment(
            __('Invoice automatically generated by Webshipr. Invoice #%1', $invoice->getIncrementId())
        )
        ->save();
    }
     ...
     * @return bool
     */
    protected function _hasItemsToInvoice(Order $order)
    {
